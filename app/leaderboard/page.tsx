...
export default function LeaderboardPage() {
  ...
  // Calculate rank for an entry (with ties)
  const calculateRank = (index: number): number => {
    if (index === 0) return 1;

    // Find how many unique scores are ahead
    let rank = 1;
  ...
  // Check if entry is tied
  const isTied = (index: number): boolean => {
    const currentScore = leaderboard[index].total_score;
    const nextScore = index < leaderboard.length - 1 ? leaderboard[index + 1].total_score : null;
    const prevScore = index > 0 ? leaderboard[index - 1].total_score : null;
    return currentScore === nextScore || currentScore === prevScore;
  };
                      ...
                      <React.Fragment key={entry.bracket_id}>
                        <motion.tr
                          ...
                          className={`border-b hover:bg-gray-50 ${
                            calculateRank(index) === 1 ? 'bg-yellow-50' : ''
                          }`}
                        >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            {calculateRank(index) === 1 && !isTied(index) && '🥇'}
                            {calculateRank(index) === 2 && !isTied(index) && '🥈'}
                            {calculateRank(index) === 3 && !isTied(index) && '🥉'}
                            <span className="font-semibold text-lg">
                              {isTied(index) ? `T-${calculateRank(index)}` : calculateRank(index)}
                            </span>
                            {(() => {
